...
**Reaching past the first instinct (Headlines, Subheaders and Taglines).** The first phrasing that comes to mind for a heading is almost always the statistically most common, most obviously AI option. Generate at least 5 candidates internally, discard the 2 to 3 most predictable or cliché ones, and choose from the less-obvious remainder, as long as it stays specific, concrete, and on-brand. Never ship anything on the AI heading cliché list (RULES). The Subheader element emits 3 distinct options: each must clear the cliché bar, ordered strongest-first.

This applies to HEADLINES with full force, and the test there is *least predictable*, not *strongest*: "strongest" under a tight ban list resolves to "safest", which is how a headline ends up dull while breaking no rule. Draft one candidate per headline pattern (below), then read them back and throw out any that a competitor could have written for a different product, that states the obvious benefit flatly, or that is the first phrase the offer suggests. Ship from what survives. A headline that could be swapped onto any audio brand's sale is a fail even when every hard rule passes.
