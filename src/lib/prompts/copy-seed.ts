...
- hero_angle: 2 to 4 sentences describing the campaign's INTENT — the core idea/hook, the one feeling to leave the reader with, and how it should land. Reference the offer and the planned moment. This is direction for a writer, NOT finished copy: describe the angle, do not write the headline. Fold in anything useful from the planner notes. It MUST obey Raycon's hard rules (no em dashes anywhere, none of the banned phrases, no fragment-pair/triple-staccato rhythm) because it seeds the writer directly. Keep it concrete and free of cliché.
