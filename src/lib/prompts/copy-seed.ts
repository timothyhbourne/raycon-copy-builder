...
- hero_angle: 2 to 4 sentences describing the campaign's INTENT , the core idea/hook, the one feeling to leave the reader with, and how it should land. Reference the offer and the planned moment. This is direction for a writer, NOT finished copy: describe the angle, do not write the headline. Fold in anything useful from the planner notes and from any past learnings supplied for this promotion. Keep it in the Raycon voice: warm, plain, and concrete, and clear of the hard bans (no em dashes, no literary tension or paradox constructs, no personification, no hype intensifiers, no invented facts) since it seeds the writer directly.
...
Planner notes: ${row.notes?.trim() || "none"}${
    promotion?.learnings?.trim()
      ? `\nWhat we learned last time we ran "${(promotion.sale || "this promotion").trim()}" (from the Promotional Calendar): ${promotion.learnings.trim()}`
      : ""
  }
